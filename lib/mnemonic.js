...
var Multi = require('multicore');
var crypto = Multi.Util('crypto');
var _ = Multi.Util('deps')._;
var $ = Multi.Util('util').preconditions;
var biterr = Multi.Util('errors');
...
var Mnemonic = function(data, wordlist) {
  ...
  if (Buffer.isBuffer(data)) {
    ...
    throw new biterr.InvalidArgument('data', 'Must be a Buffer, a string or an integer');
  }
  ent = ent || 128;
  ...
  if (ent % 32 !== 0 || ent < 128) {
    throw new biterr.InvalidArgument('ENT', 'Values must be ENT > 128 and ENT % 32 == 0');
  }
